...
func NewListener[K any, V any](handler func(K) (V, error), maxHandlers uint32, logger *slog.Logger, ctx context.Context) (*Listener[K, V], error) {

	lc := net.ListenConfig{}

	tcpListener, err := lc.Listen(ctx, "tcp", ":0") // open on the random free port
	if err != nil {
		return nil, err
	}
	...
	go func() {
		<-ctx.Done()
		logger.Info("Context expired, closing listener...")
		tcpListener.Close()
	}()
